  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setTransactions((data || []).map((t: any) => {
        // Use denormalized columns directly (fallback to old join path for safety)
        const userEmail = t.user_email || t.wallets?.users?.email || 'Unknown';
        const country = t.user_country || t.wallets?.users?.country || 'Global';
