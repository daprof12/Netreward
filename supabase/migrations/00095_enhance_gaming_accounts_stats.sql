-- 1. Add stats columns to gaming_accounts
...
ADD COLUMN IF NOT EXISTS nrt_earned NUMERIC(18,9) DEFAULT 0,
ADD COLUMN IF NOT EXISTS nrt_claimed NUMERIC(18,9) DEFAULT 0;

-- 2. Create index for performance
CREATE INDEX IF NOT EXISTS idx_gaming_accounts_campaign ON public.gaming_accounts(last_campaign_id);

-- 3. Replace process_tracking_report combining 00083 (gaming validation) + fix_flooding (batch cashback) + updating gaming_accounts stats
...
BEGIN
  ...
  IF v_campaign_category = 'gaming' THEN
    ...
    IF v_gaming_username IS NULL THEN
      -- Record session without rewards (pending review)
  ...
  -- 10. Credit user_campaigns (accumulates SP and ISP cashbacks silently)
