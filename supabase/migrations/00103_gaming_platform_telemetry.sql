-- Migration: 00103_gaming_platform_telemetry.sql
-- Description: Adds p_gaming_platform to process_tracking_report to properly attribute gaming sessions to the correct linked account based on SDK metadata.
...
CREATE OR REPLACE FUNCTION process_tracking_report(
  ...
  p_gaming_platform TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT c.sp_id, c.isp_id, c.service_id, c.network_id, c.reward_rate_per_gb, c.title,
  ...
  INTO v_campaign_sp_id, v_campaign_isp_id, v_campaign_service_id, v_campaign_network_id, 
       v_campaign_reward_rate, v_campaign_title, v_budget_remaining, v_campaign_category
  ...
  IF v_campaign_category = 'gaming' THEN
    IF p_gaming_platform IS NOT NULL THEN
      ...
      WHERE ga.user_id = v_user_id AND ga.platform = p_gaming_platform
      LIMIT 1;
    ELSE
      SELECT ga.id, ga.platform, ga.platform_username, ga.verified
      INTO v_gaming_account_id, v_gaming_platform, v_gaming_username, v_gaming_verified
    ...
    IF v_gaming_username IS NULL THEN
      INSERT INTO public.device_data_sessions (
        ...
        service_id, network_id, category, gaming_account_id
      ) VALUES (
        p_device_id, p_campaign_id, p_session_id, p_bytes_up, p_bytes_down,
        p_duration_seconds, p_session_start, p_session_end, false, 0,
        v_campaign_service_id, v_campaign_network_id, v_campaign_category, v_gaming_account_id
      ...
      RETURN jsonb_build_object(
        ...
        'message', 'Gaming campaign requires a linked gaming account for the specified platform.',
        'gaming_platform', p_gaming_platform,
        'gaming_username', NULL,
        'gaming_account_id', NULL,
        'service_id', v_campaign_service_id,
        'network_id', v_campaign_network_id,
        'category', v_campaign_category
      );
    END IF;
  END IF;
  ...
  IF v_total_nrt_earned <= 0 THEN
    RETURN jsonb_build_object(
      ...
      'gaming_account_id', v_gaming_account_id,
      'service_id', v_campaign_service_id,
      'network_id', v_campaign_network_id,
      'category', v_campaign_category
    );
