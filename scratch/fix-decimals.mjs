...
files.forEach(file => {
  ...
  if (file.includes('ScanToPay.tsx')) {
    content = content.replace(/\{detectedSession\.amountNrt\.toFixed\(2\)\} NRT/g, '<NrtAmount value={detectedSession.amountNrt} />');
    content = content.replace(/\{balanceNRT\.toFixed\(2\)\} NRT/g, '<NrtAmount value={balanceNRT} />');
  ...
  if (file.includes('DeviceDetail.tsx')) {
    content = content.replace(/\{totalNrt\.toFixed\(2\)\} NRT/g, '<NrtAmount value={totalNrt} />');
    content = content.replace(/\{Number\(app\.nrt_earned\)\.toFixed\(3\)\} NRT/g, '<NrtAmount value={app.nrt_earned} />');
    if (content !== original && !content.includes('NrtAmount')) {
      content = content.replace(/import \{ useState, useEffect \} from 'react';/, "import { useState, useEffect } from 'react';\nimport NrtAmount from '@/components/ui/NrtAmount';");
    }
  }
  ...
  if (file.includes('P2PFlow.tsx')) {
    ...
    // We'll leave the inputs as plain numbers, or if we want formatting we can use formatNrtText
  }
