  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.screenContent}>
      ...
      <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
        <Pressable style={[styles.primaryButton, { flex: 1, marginTop: 0 }]} onPress={() => router.push('/wallet/p2p')}>
          <Text style={styles.primaryButtonText}>🤝 P2P Market</Text>
        </Pressable>
        <Pressable style={[styles.primaryButton, { flex: 1, marginTop: 0, backgroundColor: '#10b981' }]} onPress={() => router.push('/wallet/scan2pay')}>
          <Text style={styles.primaryButtonText}>📷 Scan2Pay</Text>
        </Pressable>
      </View>
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.screenContent}>
      ...
      <View style={styles.settingsList}>
        {[
          { icon: '👤', label: 'Profile', desc: 'Update your info', route: '' },
          { icon: '🛡️', label: 'KYC Verification', desc: 'Verify your account', route: '/settings/kyc' },
          { icon: '🔗', label: 'Referrals', desc: 'Invite and earn', route: '/settings/referrals' },
          { icon: '📜', label: 'Transaction History', desc: 'View all activity', route: '/settings/history' },
          { icon: '🔐', label: 'Security', desc: 'Password & biometrics', route: '' },
          { icon: '🔔', label: 'Notifications', desc: 'Push & email alerts', route: '' },
        ].map((item, i) => (
          <Pressable key={i} style={styles.settingsItem} onPress={() => item.route ? router.push(item.route) : null}>
