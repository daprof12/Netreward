...
import { ChevronLeft, Target, Calendar as CalendarIcon, DollarSign, RefreshCw, Calculator, X, ChevronDown, Save, MapPin } from 'lucide-react-native';
import { useSpStore, type TargetLocation } from '@/stores/useSpStore';
...
export default function EditCampaignScreen() {
  ...
  const { services, campaigns, updateCampaign, isLoading } = useSpStore();
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Pressable onPress={() => setShowServiceDropdown(true)} style={styles.dropdownBtn}>
            <Text style={{ color: selectedService ? colors.textPrimary : colors.textSecondary }}>
              {selectedService ? `${selectedService.name} (${selectedService.category})` : 'Select a service...'}
            </Text>
      ...
      <Modal visible={showServiceDropdown} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={{ fontSize: 18, fontWeight: 'bold', color: colors.textPrimary }}>Select Service</Text>
              <Pressable onPress={() => setShowServiceDropdown(false)}><X size={24} color={colors.textPrimary} /></Pressable>
            </View>
            <ScrollView style={{ maxHeight: 300 }}>
              {services.map(s => (
                <Pressable
                  key={s.id}
                  style={[styles.serviceOption, serviceId === s.id && { backgroundColor: 'rgba(167, 139, 250, 0.1)' }]}
                  onPress={() => { setServiceId(s.id); setShowServiceDropdown(false); }}
                >
                  <Text style={{ color: colors.textPrimary, fontWeight: serviceId === s.id ? 'bold' : 'normal' }}>
                    {s.name} <Text style={{ color: colors.textSecondary, fontSize: 12 }}>({s.category})</Text>
                  </Text>
                </Pressable>
              ))}
            </ScrollView>
